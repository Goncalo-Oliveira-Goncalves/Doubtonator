...
Now, for the icons! The "Home" will have a map icon. A golden map → [assets/icons/map-icon.svg](../../assets/icons/map-icon.svg) *(missing)*. The calendar, will be a calendar (duh) → [assets/icons/calendar-icon.svg](../../assets/icons/calendar-icon.svg) *(missing)*. The Clan will be their *badge*, and don't worry, we'll talk about badges more soon → use the simplified badge from [assets/simplified/](../../assets/simplified/). The quests will be a treasure chest → [assets/icons/treasure-chest-icon.svg](../../assets/icons/treasure-chest-icon.svg) *(missing)*. And the avatar will be a little avatar, like duolingo does it → [assets/icons/avatar-icon.svg](../../assets/icons/avatar-icon.svg) *(missing)*. Please refer to the assets folder for these icons. Except the Badge, which there is 2 folders, simplified, and real. That's because we don't want too many details on a small icon. The simplified will be used when the badge is displayed small. And the real will be used when the badge is displayed on big scale, but we'll more into more details in a bit.
...
> **Suggested edit:** Also, the icon for Clan should note that the *simplified* badge version is used here specifically (the real one is only for large display contexts like the Tournaments subtab).
