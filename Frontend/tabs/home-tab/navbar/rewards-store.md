> [PENDING — experimental feature, disabled by default in v1]

> **[DOUBT]:** This whole section is marked PENDING/experimental. Should it still be fully spec'd and built (just hidden behind a feature flag), or is the design itself still in flux? Clarify so we know how much implementation effort to put in now vs. defer.
...
1. A counter, which shows your sparks (being on the left a 3d spark floating) and on the right, the actual counter -- on this section, the background is a gradient from top left to bottom right of the section container, from pink to neon pink. The 3d spark (and other sparks) contrast in color.

> **[DOUBT]:** "Pink to neon pink" gradient — these colors have no hex values or semantic tokens in brand-guidelines.md. What are the specific values? Also, "3D spark floating" — is there an asset for this in the assets folder, or does it need to be created? What does it look like? Upon transition from section 1 to 2, the background gradient smoothly goes to the white background of the sections 2 & 3.
